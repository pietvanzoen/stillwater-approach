...
- **Module globals (exception)**: Playdate's `import` doesn't return values, so shared modules (`Strings`, `Constants`, etc.) assign to named globals. Declare them in `.luacheckrc` under `globals` and annotate the file with `-- luacheck: globals <Name>`.
